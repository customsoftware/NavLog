//
//  HSISwiftUIView.swift
//  NavLog
//
//  Created by Kenneth Cluff on 11/27/23.
//
...
struct InstrumentView: View {
    ...
    @State var trueHeading: Double = Core.services.gpsEngine.heading
    
    var body: some View {
        DGView()
            .rotationEffect(.degrees(trueHeading), anchor: .center)
            .onAppear(perform: {
                Core.services.gpsEngine.locationManger.startUpdatingHeading()
                print("Heading services running")
            })
            .onDisappear(perform: {
                Core.services.gpsEngine.locationManger.stopUpdatingHeading()
                print("Heading services shut down")
            })
    }
}
